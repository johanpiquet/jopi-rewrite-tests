...
function addCacheUpdateDateComment(text) {
    const comment = "Jopi - Cache updated at: " + new Date().toISOString();
    text += "\n\n<!--\n" + comment + "\n-->";
    return text;
}
function replaceSomeHtml(html, req) {
...
await createServer(async req => {
    ...
    res = await req.hookIfCookie(res, "myCookie", cookie => cookie === "admin", text => text + "<!-- With cookie ! -->");
    return res;
});
